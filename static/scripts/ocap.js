...
async function getWorldByName (worldName) {
	...
	let defaultMap = {
		...
		"worldName": "NOT FOUND",
		"worldSize": 16384,
		"bounds": [0, 0, 0.27596245528151697, 0.2759613883145673],
		"center": [0, 0],
		// "imageSize": 16384,
		// "multiplier": 1,
		// "maxZoom": 6,
		...
		"author": "Bohemia Interactive and 3rd Party Developers"
	};

	const url = `https://styles.ocap2.com/${worldName}.json`;


	const cloudMapRes = await fetch(url);
	if (cloudMapRes.status === 200) {
		try {
			worldObject = Object.assign(defaultMap, (await cloudMapRes.json()).metadata, { _useCloudTiles: true });
			return Promise.resolve(worldObject);
		} catch (error) {
...
function initMap (world) {
	...
	map.attributionControl.addAttribution('Arma 3 &copy; Bohemia Interactive');
	...
	worldObject.gl = L.maplibreGL({
		style: `https://styles.ocap2.com/${world.worldName}.json`,
		// style: {
		// 	"version": 8,
		// 	"sources": {
		// 		"raster-tiles": {
		// 			"type": "raster",
		// 			"url": `https://a3tiles.indigofox.dev/styles/${world.worldName}.json`,
		// 			"tileSize": 256
		// 		}
		// 	},
		// 	"layers": [{
		...
		// 	}]
		// },
		attribution: 'World created by ' + worldObject.author,
		noWrap: true,
	}).addTo(map);


	// L.tileLayer(`https://a3tiles.indigofox.dev/styles/${worldName}/{z}/{x}/{y}.png`, {
	// 	attribution: 'World created by ' + worldObject.author,
	// 	minZoom: 0,
	// 	maxZoom: 24,
	// 	noWrap: true,
	...
	map.setView(worldObject.center, 12)
	...
	map.on("overlayadd", (event) => {
		...
		switch (event.name) {
			...
			case "Editor/Briefing Markers": {
				...
				systemMarkersLayerGroup.getLayers().forEach(layer => {
					layer.remove()
				})
				// }
				break;
